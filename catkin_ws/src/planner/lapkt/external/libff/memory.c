...
PseudoAction *new_PseudoAction( MixedOperator *thisOperator )

{
  ...
  result->pddloperator = thisOperator->pddloperator;
  for ( i = 0; i < thisOperator->pddloperator->num_vars; i++ ) {
    result->inst_table[i] = thisOperator->inst_table[i];
  }

  result->preconds = thisOperator->preconds;
  result->num_preconds = thisOperator->num_preconds;

  result->numeric_preconds_comp = thisOperator->numeric_preconds_comp;
  result->numeric_preconds_lh = thisOperator->numeric_preconds_lh;
  result->numeric_preconds_rh = thisOperator->numeric_preconds_rh;
  result->num_numeric_preconds = thisOperator->num_numeric_preconds;
...
/* TODO (DRJ)  Added NULL reset to all freed pointers */
...
void free_Everything() {
  ...
  printf("#######################################################################'\n");
  printf("#                      CLEAN UP FROM LAST RUN                         #\n");
  printf("#######################################################################\n");


  printf("Checkin 'glnf_goal_comp'\n");
  if (glnf_goal_comp) {
    printf("  Freeing 'glnf_goal_comp'\n");
    free(glnf_goal_comp);
    glnf_goal_comp = NULL;
  }

  printf("Checkin 'glnf_goal_lh'\n");
  if(glnf_goal_lh) {
    printf("  Freeing 'glnf_goal_lh'\n");
    free(glnf_goal_lh);
    glnf_goal_lh = NULL;
  }

  printf("Checkin 'grelevant_fluents_name[]'\n");
  for ( i = 0; i < gnum_relevant_fluents ; i++ ) {
    if(grelevant_fluents_name[i]) {
      printf("  Freeing 'grelevant_fluents_name[%d]'\n",i);
      free(grelevant_fluents_name[i]);
      grelevant_fluents_name[i] = NULL;
    }
  }

  printf("Checkin 'glogic_goal'\n");
  if (glogic_goal) {
    printf("  Freeing 'glogic_goal'\n");
    free(glogic_goal);
    glogic_goal = NULL;
  }

  printf("Checkin 'gnumeric_goal_comp'\n");
  if (gnumeric_goal_comp) {
    printf("  Freeing 'gnumeric_goal_comp'\n");
    free(gnumeric_goal_comp);
    gnumeric_goal_comp = NULL;
  }

  printf("Checkin 'gnumeric_goal_lh'\n");
  if (gnumeric_goal_lh) {
    printf("  Freeing 'gnumeric_goal_lh'\n");
    free(gnumeric_goal_lh);
    gnumeric_goal_lh = NULL;

  }

  printf("Checkin 'gft_conn'\n");
  if (gft_conn) {
    /* gnum_ft_conn = gnum_relevant_facts */
    for ( i = 0; i < gnum_ft_conn ; i++ ) {
      if (gft_conn[i].PC) {
        printf("    Freeing 'gft_conn[%d].PC'\n",i);
        free(gft_conn[i].PC);
        gft_conn[i].PC = NULL;
      }
      if (gft_conn[i].A) {
        printf("    Freeing 'gft_conn[%d].A'\n",i);
        free(gft_conn[i].A);
        gft_conn[i].A = NULL;
      }
      if (gft_conn[i].D) {
        printf("    Freeing 'gft_conn[%d].D'\n",i);
        free(gft_conn[i].D);
        gft_conn[i].D = NULL;
      }
    }
    printf("  Freeing 'gft_conn'\n");
    free(gft_conn);
    gft_conn = NULL;
  }

  printf("Checkin 'gfl_conn'\n");
  if(gfl_conn) {
    for ( i = 0; i < gnum_real_fl_conn ; i++ ) {
      if(gfl_conn[i].lnf_F) {
        printf("    Freeing 'gfl_conn[%d].lnf_F'\n",i);
        free(gfl_conn[i].lnf_F);
        gfl_conn[i].lnf_F = NULL;
      }
      if(gfl_conn[i].lnf_C) {
        printf("    Freeing 'gfl_conn[%d].lnf_C'\n",i);
        free(gfl_conn[i].lnf_C);
        gfl_conn[i].lnf_C = NULL;
      }
      if(gfl_conn[i].PC) {
        printf("    Freeing 'gfl_conn[%d].PC'\n",i);
        free(gfl_conn[i].PC);
        gfl_conn[i].PC = NULL;
      }
      if(gfl_conn[i].IN) {
        printf("    Freeing 'gfl_conn[%d].IN'\n",i);
        free(gfl_conn[i].IN);
        gfl_conn[i].IN = NULL;
      }
      if(gfl_conn[i].IN_fl_) {
        printf("    Freeing 'gfl_conn[%d].IN_fl_'\n",i);
        free(gfl_conn[i].IN_fl_);
        gfl_conn[i].IN_fl_ = NULL;
      }
      if(gfl_conn[i].IN_c) {
        printf("    Freeing 'gfl_conn[%d].IN_c'\n",i);
        free(gfl_conn[i].IN_c);
        gfl_conn[i].IN_c = NULL;
      }
      if(gfl_conn[i].AS) {
        printf("    Freeing 'gfl_conn[%d].AS'\n",i);
        free(gfl_conn[i].AS);
        gfl_conn[i].AS = NULL;
      }
      if(gfl_conn[i].AS_fl_) {
        printf("    Freeing 'gfl_conn[%d].AS_fl_'\n",i);
        free(gfl_conn[i].AS_fl_);
        gfl_conn[i].AS_fl_ = NULL;
      }
      if(gfl_conn[i].AS_c) {
        printf("    Freeing 'gfl_conn[%d].AS_c'\n",i);
        free(gfl_conn[i].AS_c);
        gfl_conn[i].AS_c = NULL;
      }
      if(gfl_conn[i].def) {
        printf("    Freeing 'gfl_conn[%d].def'\n",i);
        free(gfl_conn[i].def);
        gfl_conn[i].def = NULL;
      }
      if(gfl_conn[i].level) {
        printf("    Freeing 'gfl_conn[%d].level'\n",i);
        free(gfl_conn[i].level);
        gfl_conn[i].level = NULL;
      }
    }
    printf("  Freeing 'gfl_conn'\n");
    free(gfl_conn);
    gfl_conn = NULL;
  }

  printf("Checkin 'gop_conn'\n");
  if(gop_conn) {
    for ( i = 0; i < gnum_op_conn ; i++ ) {
      if(gop_conn[i].E) {
        printf("    Freeing 'gop_conn[%d].E'\n",i);
        free(gop_conn[i].E);
        gop_conn[i].E = NULL;
      }
    }
    printf("  Freeing 'gop_conn'\n");
    free(gop_conn);
    gop_conn = NULL;
  }

  printf("Checkin 'gef_conn'\n");
  if(gef_conn) {
    for ( i = 0; i < gnum_ef_conn ; i++ ) {
      if(gef_conn[i].PC) {
        printf("    Freeing 'gef_conn[%d].PC'\n",i);
        free(gef_conn[i].PC);
        gef_conn[i].PC = NULL;
      }
      if(gef_conn[i].f_PC_comp) {
        printf("    Freeing 'gef_conn[%d].f_PC_comp'\n",i);
        free(gef_conn[i].f_PC_comp);
        gef_conn[i].f_PC_comp = NULL;
      }
      if(gef_conn[i].f_PC_fl) {
        printf("    Freeing 'gef_conn[%d].f_PC_fl'\n",i);
        free(gef_conn[i].f_PC_fl);
        gef_conn[i].f_PC_fl = NULL;
      }
      if(gef_conn[i].f_PC_c) {
       printf("    Freeing 'gef_conn[%d].f_PC_c'\n",i);
       free(gef_conn[i].f_PC_c);
        gef_conn[i].f_PC_c = NULL;
      }
      if(gef_conn[i].f_PC_direct_comp) {
        printf("    Freeing 'gef_conn[%d].f_PC_direct_comp'\n",i);
        free(gef_conn[i].f_PC_direct_comp);
        gef_conn[i].f_PC_direct_comp = NULL;
      }
      if(gef_conn[i].f_PC_direct_c) {
        printf("    Freeing 'gef_conn[%d].f_PC_direct_c'\n",i);
        free(gef_conn[i].f_PC_direct_c);
        gef_conn[i].f_PC_direct_c = NULL;
      }
      if(gef_conn[i].A) {
        printf("    Freeing 'gef_conn[%d].A'\n",i);
        free(gef_conn[i].A);
        gef_conn[i].A = NULL;
      }
      if(gef_conn[i].D) {
        printf("    Freeing 'gef_conn[%d].D'\n",i);
        free(gef_conn[i].D);
        gef_conn[i].D = NULL;
      }
      if(gef_conn[i].IN_fl) {
        printf("    Freeing 'gef_conn[%d].IN_fl'\n",i);
        free(gef_conn[i].IN_fl);
        gef_conn[i].IN_fl = NULL;
      }
      if(gef_conn[i].IN_fl_) {
        printf("    Freeing 'gef_conn[%d].IN_fl_'\n",i);
        free(gef_conn[i].IN_fl_);
        gef_conn[i].IN_fl_ = NULL;
      }
      if(gef_conn[i].IN_c) {
        printf("    Freeing 'gef_conn[%d].IN_c'\n",i);
        free(gef_conn[i].IN_c);
        gef_conn[i].IN_c = NULL;
      }
      if(gef_conn[i].AS_fl) {
        printf("    Freeing 'gef_conn[%d].AS_fl'\n",i);
        free(gef_conn[i].AS_fl);
        gef_conn[i].AS_fl = NULL;
      }
      if(gef_conn[i].AS_fl_) {
        printf("    Freeing 'gef_conn[%d].AS_fl_'\n",i);
        free(gef_conn[i].AS_fl_);
        gef_conn[i].AS_fl_ = NULL;
      }
       if(gef_conn[i].AS_c) {
        printf("    Freeing 'gef_conn[%d].AS_c'\n",i);
        free(gef_conn[i].AS_c);
        gef_conn[i].AS_c = NULL;
      }
      if(gef_conn[i].I) {
        printf("    Freeing 'gef_conn[%d].I'\n",i);
        free(gef_conn[i].I);
        gef_conn[i].I = NULL;
      }
    }
    printf("  Freeing 'gef_conn'\n");
    free(gef_conn);
    gef_conn = NULL;
  }

  printf("Checkin 'gmneed_start_D'\n");
  if(gmneed_start_D) {
    printf("  Freeing 'gmneed_start_D'\n");
    free(gmneed_start_D);
    gmneed_start_D = NULL;
  }

  printf("Checkin 'gmneed_start_V'\n");
  if(gmneed_start_V) {
    printf("  Freeing 'gmneed_start_V'\n");
    free(gmneed_start_V);
    gmneed_start_V = NULL;
  }

  printf("Checkin 'gflogic_goal'\n");
  if(gflogic_goal) {
    printf("  Freeing 'gflogic_goal'\n");
    free(gflogic_goal);
    gflogic_goal = NULL;
  }

  printf("Checkin 'gfnumeric_goal_comp'\n");
  if(gfnumeric_goal_comp) {
    printf("  Freeing 'gfnumeric_goal_comp'\n");
    free(gfnumeric_goal_comp);
    gfnumeric_goal_comp = NULL;
  }

  printf("Checkin 'gfnumeric_goal_fl'\n");
  if(gfnumeric_goal_fl) {
    printf("  Freeing 'gfnumeric_goal_fl'\n");
    free(gfnumeric_goal_fl);
    gfnumeric_goal_fl = NULL;
  }

  printf("Checkin 'gfnumeric_goal_c'\n");
  if(gfnumeric_goal_c) {
    printf("  Freeing 'gfnumeric_goal_c'\n");
    free(gfnumeric_goal_c);
    gfnumeric_goal_c = NULL;
  }

  printf("Checkin 'gfnumeric_goal_direct_comp'\n");
  if(gfnumeric_goal_direct_comp) {
    printf("  Freeing 'gfnumeric_goal_direct_comp'\n");
    free(gfnumeric_goal_direct_comp);
    gfnumeric_goal_direct_comp = NULL;
  }

  printf("Checkin 'gfnumeric_goal_direct_c'\n");
  if(gfnumeric_goal_direct_c) {
    printf("  Freeing 'gfnumeric_goal_direct_c'\n");
    free(gfnumeric_goal_direct_c);
    gfnumeric_goal_direct_c = NULL;
  }

  printf("Checkin 'gfull_initial'\n");
  if(gfull_initial) {
    printf("  Freeing 'gfull_initial'\n");
    free(gfull_initial);
    gfull_initial= NULL;
  }

  printf("Checkin 'gfull_fluents_initial'\n");
  if(gfull_fluents_initial) {
    printf("  Freeing 'gfull_fluents_initial'\n");
    free(gfull_fluents_initial);
    gfull_fluents_initial= NULL;
  }

  printf("Checkin 'ginitial_predicate'\n");
  if(ginitial_predicate) {
    for ( i = 0; i < gnum_predicates ; i++ ) {
      if(ginitial_predicate[i]) {
        printf("    Freeing 'ginitial_predicate[%d]'\n",i);
  ...
  printf("Checkin 'gnum_initial_predicate'\n");
  if(gnum_initial_predicate) {
    printf("  Freeing 'gnum_initial_predicate'\n");
    free(gnum_initial_predicate);
    gnum_initial_predicate= NULL;
  }

  printf("Checkin 'ginitial_function'\n");
  if(ginitial_function) {
    for ( i = 0; i < gnum_functions ; i++ ) {
      if(ginitial_function[i]) {
        printf("    Freeing 'ginitial_function[%d]'\n",i);
        free(ginitial_function[i]);
        ginitial_function[i] = NULL;
      }
    }
    printf("  Freeing 'ginitial_function'\n");
    free(ginitial_function);
    ginitial_function= NULL;
  }

  printf("Checkin 'gnum_initial_function'\n");
  if(gnum_initial_function) {
    printf("  Freeing 'gnum_initial_function'\n");
  ...
  printf("Checkin 'geasy_operators'\n");
  if(geasy_operators) {
    printf("  Freeing 'geasy_operators'\n");
    free(geasy_operators);
  ...
  printf("Checkin 'gassign_influence'\n");
  if(gassign_influence) {
    for ( i = 0; i < gnum_real_fl_conn ; i++ ) {
      if(gassign_influence[i]) {
        printf("    Freeing 'gassign_influence[%d]'\n",i);
        free(gassign_influence[i]);
        gassign_influence[i] = NULL;
      }
    }
    printf("  Freeing 'gassign_influence'\n");
    free(gassign_influence);
    gassign_influence= NULL;
  }

  printf("Checkin 'gTassign_influence'\n");
  if(gTassign_influence) {
    for ( i = 0; i < gnum_real_fl_conn ; i++ ) {
      if(gTassign_influence[i]) {
        printf("    Freeing 'gTassign_influence[%d]'\n",i);
        free(gTassign_influence[i]);
        gTassign_influence[i] = NULL;
      }
    }
    printf("  Freeing 'gTassign_influence'\n");
