...
{
          /*  SET TO NULL     gorig_constant_list  gorig_goal_facts(?) gfunctions_and_types      */
  char *tmp = NULL;
  TypedList *tyl;
  TypedListList *tyll;
...
*/
  lnum_types = 0;
  for ( tyl = gparse_types; tyl; tyl = tyl->next ) {    /* :types INTERSECTION AGENT (Domain#3) */
    if ( get_type( tyl->name ) == -1 ) {                /* OBJECT also comes in */
  ...
  for ( tyl = gparse_constants; tyl; tyl = tyl->next ) {    /* :constants robot - agent (Domain#4) */
    if ( tyl->type->next ) {
      tmp = new_Token( MAX_LENGTH );
      strcpy( tmp, EITHER_STR );
      for ( tl = tyl->type; tl; tl = tl->next ) {
  ...
  for ( tyl = gparse_objects; tyl; tyl = tyl->next ) {    /* :objects X11 X12 ... X66 (Problem#3-9)*/
    if ( tyl->type->next ) {
      tmp = new_Token( MAX_LENGTH );
      strcpy( tmp, EITHER_STR );
      for ( tl = tyl->type; tl; tl = tl->next ) {
      ...
      tmp = copy_Token( tyl->type->item );                /* - intersection  (Problem#9)*/
    }
    if ( (n = get_type( tmp )) == -1 ) {
      tyl->n = lnum_types;
      ltype_names[lnum_types++] = copy_Token( tmp );
  ...
  for ( tyll = gparse_predicates; tyll; tyll = tyll->next ) {     /* :predicates AT, ADJOIN, BLOCKED (Domain#5-11)*/
    for ( tyl = tyll->args; tyl; tyl = tyl->next ) {              /*  tyll = 'AT' etc */
      if ( tyl->type->next ) {                                    /*  tyl  = parameter ('?x'),('?y') */
        tmp = new_Token( MAX_LENGTH );
        strcpy( tmp, EITHER_STR );
        for ( tl = tyl->type; tl; tl = tl->next ) {
          strcat( tmp, CONNECTOR );
  ...
  collect_type_names_in_pl( gorig_goal_facts );                 /* gorig_goal_facts->sons->atom->item = "AT"  (Problem#11 maybe) */

  for ( po = gloaded_ops; po; po = po->next ) {                 /*  :action MOVE  (Domain#14-30) */
    collect_type_names_in_pl( po->preconds );                   /*  :preconditions (Domain#16-22) */
    collect_type_names_in_pl( po->effects );
    for ( tyl = po->parse_params; tyl; tyl = tyl->next ) {      /*  :parameters (Domain#15) */
      if ( tyl->type->next ) {
        tmp = new_Token( MAX_LENGTH );
        strcpy( tmp, EITHER_STR );
        for ( tl = tyl->type; tl; tl = tl->next ) {
  ...
  make_either_ty_in_pl( gorig_goal_facts );                 /* gorig_goal_facts->sons->atom->item = "AT"  (Problem#11 maybe) */
  for ( po = gloaded_ops; po; po = po->next ) {             /* actions*/
    make_either_ty_in_pl( po->preconds );                   /*  preconditions */
    make_either_ty_in_pl( po->effects );                    /*  effects */
    for ( tyl = po->parse_params; tyl; tyl = tyl->next ) {  /*  parameters */
      make_either_ty( tyl );
    }
  }
   ...
   */
  for ( tyl = gparse_constants; tyl; tyl = tyl->next ) {              /* :constants ROBOT (Domain#4) */
    fl = new_FactList();
   ...
   */
  normalize_tyl_in_pl( &gorig_goal_facts );                             /* Goal AT Robot X22 (Problem#119-123) */
  for ( po = gloaded_ops; po; po = po->next ) {                         /* Actions */
...
Bool make_conjunction_of_atoms( PlNode **node )
...

{
  ...
  case ALL:
  case EX:
    if ( !(n->atom) || !(n->atom->next ) || n->atom->next->next != NULL ) {
      return FALSE;
    }
  ...
  case FAL:
    ...
    return TRUE;
  case COMP:
    if ( n->sons != NULL || n->atom != NULL || n->lh == NULL || n->rh == NULL || n->comp < 0 ) {
      return FALSE;
    }
    return TRUE;
  default:
...

{
  ...
  if ( (*n)->connective != AND ) {
    if ( !is_eff_literal( *n ) && (*n)->connective != ALL && (*n)->connective != WHEN ) {
      return FALSE;
    }
  ...
  for ( i = (*n)->sons; i; i = i->next ) {
    ...
    if ( i->connective == ALL ) {
      for ( j = i->sons; j && j->connective == ALL; j = j->sons ) {
        if ( !j->atom || !j->atom->next || j->atom->next->next != NULL ) {
          return FALSE;
        }
      }
      if ( !j ) {
...

{
  ...
  if ( n->connective == NOT ) {
    if ( !n->sons || n->sons->connective != ATOM || !n->sons->atom ) {
      return FALSE;
    }
    return TRUE;
  }
...

{
  ...
  if ( (*n)->connective != AND ) {
    if ( (*n)->connective == NOT ) {
      if ( !((*n)->sons) || (*n)->sons->connective != ATOM ) {
        return FALSE;
      }
      tmp = new_PlNode( NOT );
      tmp->sons = (*n)->sons;
