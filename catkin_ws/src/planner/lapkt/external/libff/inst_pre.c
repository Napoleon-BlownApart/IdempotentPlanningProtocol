...

{
  ...
  for ( factList = gorig_constant_list; factList; factList = factList->next ) {
    if ( (type_num = position_in_types_table( factList->item->next->item )) == -1 ) {
      if ( gnum_types == MAX_TYPES ) {
        printf("\ntoo many types! increase MAX_TYPES (currently %d)\n\n", MAX_TYPES);
        exit( 1 );
      }
      gtype_names[gnum_types] = new_Token( strlen( factList->item->next->item ) + 1 );
      strcpy( gtype_names[gnum_types], factList->item->next->item );
    ...
    if ( (c_num = position_in_constants_table( factList->item->item )) == -1 ) {
      if ( gnum_constants == MAX_CONSTANTS ) {
        printf("\ntoo many constants! increase MAX_CONSTANTS (currently %d)\n\n", MAX_CONSTANTS);
        exit( 1 );
      }
      gconstants[gnum_constants] = new_Token( strlen( factList->item->item ) + 1 );
      strcpy( gconstants[gnum_constants], factList->item->item );
  ...
  for ( factList = gpredicates_and_types; factList; factList = factList->next ) {
    if ( (p_num = position_in_predicates_table( factList->item->item )) != -1 ) {
    ...
    gpredicates[gnum_predicates] = new_Token( strlen( factList->item->item ) + 1 );
    strcpy( gpredicates[gnum_predicates], factList->item->item );
    ar = 0;
    for ( t = factList->item->next; t; t = t->next ) {
      if ( (type_num = position_in_types_table( t->item )) == -1 ) {
        printf("\npredicate %s is declared to use unknown or empty type %s\n\n", factList->item->item, t->item);
        break;
        /*	exit( 1 ); */
      }
      if ( ar == MAX_ARITY ) {
  ...
  for ( factList = gfunctions_and_types; factList; factList = factList->next ) {
    if ( (p_num = position_in_functions_table( factList->item->item )) != -1 ) {
    ...
    gfunctions[gnum_functions] = new_Token( strlen( factList->item->item ) + 1 );
    strcpy( gfunctions[gnum_functions], factList->item->item );
    ar = 0;
    for ( t = factList->item->next; t; t = t->next ) {
      if ( (type_num = position_in_types_table( t->item )) == -1 ) {
...

{
  ...
  for ( i = 1; i < gnum_types; i++ ) {
    if ( str == gtype_names[i] || (strcmp( str, gtype_names[i] ) == SAME) ) {
      break;
    }
  }
...

{
  ...
  for ( i=0; i<gnum_constants; i++ ) {
    if ( str == gconstants[i] || strcmp( str, gconstants[i] ) == SAME ) {
      break;
    }
  }
...

{
  ...
  for ( i = 0; i < gnum_predicates; i++ ) {
    if ( str == gpredicates[i] || strcmp( str, gpredicates[i] ) == SAME ) {
      break;
    }
  }
...

{
  ...
  if ( gorig_initial_facts ) {
    ...
    gfull_fluents_initial = ( FluentValue * ) calloc( sum, sizeof( FluentValue ));
    ...
    for ( n = gorig_initial_facts->sons; n; n = n->next ) {
      if ( n->connective == ATOM ) {
        ...
        make_Fluent( &(gfull_fluents_initial[gnum_full_fluents_initial].fluent), n->lh->atom, 0 );
        gfull_fluents_initial[gnum_full_fluents_initial].value = ( float ) strtod( n->rh->atom->item, NULL);
        gnum_full_fluents_initial++;
      }
    }
    /* TODO (DRJ) Lets not clear gorig_initial_facts as they
                  can be useful downstream.
    */
...
/*    gorig_initial_facts = NULL; */
  ...
  if ( gparse_metric != NULL ) {
    if ( !gcmd_line.optimize ) {
      ...
      if ( strcmp( gparse_optimization, "MINIMIZE" ) != SAME && strcmp( gparse_optimization, "MAXIMIZE" ) != SAME ) {
        if ( gcmd_line.display_info ) {
  ...
  for ( o = gloaded_ops; o; o = o->next ) {
    ...
    for ( ff = o->params; ff; ff = ff->next ) {
      if ( (type_num = position_in_types_table( ff->item->next->item )) == -1 ) {
        printf("\nwarning: parameter %s of op %s has unknown or empty type %s. skipping op", ff->item->item, o->name, ff->item->next->item);
        break;
      }
    ...
    goperators[gnum_operators++] = tmp; /* First Time through = 0 */
  }
...

{
  ...
  for ( t = n->atom->next; t; t = t->next ) {
    if ( t->item[0] == '?' ) {
      ...
      if ( lvar_types[i] != gpredicates_args_type[f->predicate][m] && !is_subtype( lvar_types[i], gpredicates_args_type[f->predicate][m] ) ) {
...

{
  ...
  for ( t = atom->next; t; t = t->next ) {
    if ( t->item[0] == '?' ) {
      ...
      if ( lvar_types[i] != gfunctions_args_type[f->function][m] && !is_subtype( lvar_types[i], gfunctions_args_type[f->function][m] ) ) {
...

{
  ...
  case FHEAD:
    ...
    break;
  default:
    printf("\n\nmake expnode: wrong specifier %d", p->connective);
    exit( 1 );
  }
...

{
  ...
  for ( n = p; n && n->connective == ALL; n = n->sons ) {
    ...
    for ( i = 0; i < num_vars + tmp->num_vars; i++ ) {
      if ( lvar_names[i] == n->atom->item || strcmp( lvar_names[i], n->atom->item ) == SAME ) {
  ...
  for ( m = n->sons->next->sons; m; m = m->next ) {
    if ( m->connective == NEF ) {
      if ( tmp->numeric_effects != NULL ) {
        ...
        make_Fluent( &(tmp->numeric_effects->prev->fluent), m->lh->atom, num_vars + tmp->num_vars );
        ...
        make_Fluent( &(tmp->numeric_effects->fluent), m->lh->atom, num_vars + tmp->num_vars );
...

{
  ...
  if ( gcmd_line.display_info == 105 ) {
    ...
    for ( i = 0; i < gnum_predicates; i++ ) {
      printf("\n%3d --> %s: ", i, gpredicates[i]);
      printf(" is %s, %s", gis_added[i] ? "ADDED" : "NOT ADDED", gis_deleted[i] ? "DELETED" : "NOT DELETED");
    }
    printf("\n\nfunctions inertia info:");
    for ( i = 0; i < gnum_functions; i++ ) {
      printf("\n%3d --> %s: ", i, gfunctions[i]);
      printf(" is %s", gis_changed[i] ? "CHANGED" : "NOT CHANGED");
    }
    printf("\n\n");
  }
...

{
  ...
  for ( i = 0; i < gnum_predicates; i++ ) {
    if ( !gis_added[i] && !gis_deleted[i] && garity[i] == 1 ) {
      if ( gnum_types == MAX_TYPES ) {
  ...
  ginitial_function = ( FluentValue ** ) calloc( gnum_functions, sizeof( FluentValue * ) );
  ...
  for ( i = 0; i < gnum_functions; i++ ) {
    ginitial_function[i] = ( gnum_initial_function[i] ? ( FluentValue * ) calloc( gnum_initial_function[i], sizeof( FluentValue ) ) : NULL);
    gnum_initial_function[i] = 0;
  }
  gf_initial = NULL;
  gnum_f_initial = 0;
  ...
  for ( i = 0; i < gnum_full_fluents_initial; i++ ) {
    ...
    for ( j = 0; j < gf_arity[p]; j++ ) {
      ginitial_function[p][gnum_initial_function[p]].fluent.args[j] = gfull_fluents_initial[i].fluent.args[j];
    }
    ginitial_function[p][gnum_initial_function[p]].value = gfull_fluents_initial[i].value;
    gnum_initial_function[p]++;
    if ( gis_changed[p] ) {
...

{
  ...
  case MU:
  case DI:
    if ( var_used_in_exp( code_var, n->leftson ) || var_used_in_exp( code_var, n->rightson ) ) {
      return TRUE;
    }
    return FALSE;
  case MINUS:
  ...
  case FHEAD:
    ...
    return FALSE;
  default:
    printf("\n\nvar used in expnode: wrong specifier %d", n->connective);
    exit( 1 );
  }
...

{
  ...
  case FHEAD:
    ...
    break;
  default:
    printf("\n\ndecr inf vars in expnode: wrong specifier %d", n->connective);
    exit( 1 );
  }
...

{
  ...
  case EX:
    simplify_wff( &((*w)->son) );
    if ( (*w)->son->connective == TRU || (*w)->son->connective == FAL ) {
      (*w)->connective = (*w)->son->connective;
      free( (*w)->son );
      (*w)->son = NULL;
      if ( (*w)->var_name ) {
  ...
  case COMP:
    simplify_exp( &((*w)->lh) );
    simplify_exp( &((*w)->rh) );
    if ( (*w)->lh->connective != NUMBER || (*w)->rh->connective != NUMBER ) {
...

{
  ...
  case AD:
    ...
    if ( (*n)->leftson->connective != NUMBER || (*n)->rightson->connective != NUMBER ) {
      break;
    }
    (*n)->connective = NUMBER;
  ...
  case FHEAD:
    ...
    for ( j = 0; j < gnum_initial_function[f]; j++ ) {
      for ( k = 0; k < gf_arity[f]; k++ ) {
        if ( ginitial_function[f][j].fluent.args[k] != (*n)->fluent->args[k] )
          break;
      }
    ...
    break;
  default:
    printf("\n\nsimplify expnode: wrong specifier %d", (*n)->connective);
    exit( 1 );
  }
...

{
  ...
  case FHEAD:
    ...
    break;
  default:
    printf("\n\nreplace var with const in expnode: wrong specifier %d", (*n)->connective);
    exit( 1 );
  }
...

{
  ...
  case FHEAD:
    ...
    break;
  default:
    printf("\n\ncopy expnode: wrong specifier %d", n->connective);
    exit( 1 );
  }
...

{
  ...
  for ( i = 0; i < garity[f1->predicate]; i++ ) {
    if ( f1->args[i] >= 0 ) {
      if ( f2->args[i] >= 0 && f1->args[i] != f2->args[i] ) {
        return FALSE;
      }
    }
  }
  ...
  case OR:
    ...
    for ( i = (*w)->sons; i && i->next; i = i->next ) {
      ...
      while ( j ) {
        ...
        if ( i->connective == NOT && are_identical_ATOMs( i->son, j ) ) {
          (*w)->connective = ( (*w)->connective == AND ) ? FAL : TRU;
          free_WffNode( (*w)->son );
          (*w)->son = NULL;
          return;
        }
        if ( j->connective == NOT && are_identical_ATOMs( i, j->son ) ) {
          (*w)->connective = ( (*w)->connective == AND ) ? FAL : TRU;
          free_WffNode( (*w)->son );
          (*w)->son = NULL;
          return;
...

{
  ...
  if ( w1->connective != ATOM || w2->connective != ATOM ) {
    return FALSE;
  }
...

{
  ...
  case NOT:
    ...
    if ( (*w)->son->connective == AND || (*w)->son->connective == OR ) {
...

{
  ...
  for ( i = 0; i < gnum_operators; i++ ) {    /* First time through this = 1 */
    if ( goperators[i]->hard ) {
      ghard_operators[gnum_hard_operators++] = goperators[i]; /* First time through this = 0 */
      continue;
    }
...

{
  ...
  case AND:
    for ( i = w->sons; i; i = i->next ) {
      if ( i->connective == ATOM || i->connective == COMP ) {
        continue;
      }
      return -1;
    }
  ...
  case OR:
    for ( i = w->sons; i; i = i->next ) {
      s++;
      if ( i->connective == ATOM || i->connective == COMP || ( i->connective == AND && is_dnf( i ) != -1 ) ) {
        continue;
      }
      return -1;
    }
