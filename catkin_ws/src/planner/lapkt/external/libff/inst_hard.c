...

{
  ...
  case AND:
    ...
    if(mn!=0) {
      ...
      tmp2->numeric_preconds_comp = NULL;
      tmp2->numeric_preconds_lh = NULL;
      tmp2->numeric_preconds_rh = NULL;
    }
...
Effect *instantiate_Effect( Effect *effect )
  ...
  for ( i = effect; i; i = i->next ) {
    ...
    for ( ne = i->numeric_effects; ne; ne = ne->next ) {
      ...
      for ( j = 0; j < gf_arity[ttt->fluent.function]; j++ ) {
        ...
        if ( ttt->fluent.args[j] < 0 && linst_table[DECODE_VAR( ttt->fluent.args[j] )] != -1 ) {
...

{
  ...
  case COMP:
    ...
    if ( res->lh->connective != NUMBER || res->rh->connective != NUMBER ) {
  ...
  case FHEAD:
    ...
    for ( j = 0; j < gnum_initial_function[f]; j++ ) {
      for ( k = 0; k < gf_arity[f]; k++ ) {
        if ( ginitial_function[f][j].fluent.args[k] != (*n)->fluent->args[k] ) break;
      }
...

{
  ...
  ghard_templates = ( PseudoAction_pointer * ) calloc( gnum_hard_mixed_operators, sizeof ( PseudoAction_pointer ) );
  gnum_hard_templates = 0;
...
void create_hard_pseudo_effects( PseudoAction* pseudoAction, Effect* effect, int curr_var )

{
  ...
  if ( curr_var < effect->num_vars ) {
    par = pseudoAction->pddloperator->num_vars + curr_var;

    t = effect->var_types[curr_var];
    for ( i = 0; i < gtype_size[t]; i++ ) {
      linst_table[par] = gtype_consts[t][i];

      create_hard_pseudo_effects( pseudoAction, effect, curr_var + 1 );
  ...
  tmp1 = instantiate_wff( effect->conditions );
   ...
   */
  if ( is_dnf( tmp1 ) == -1 ) {
    printf("\n\nILLEGAL DNF %s AFTER INSTANTIATION\n\n", pseudoAction->pddloperator->name);
    print_Wff( tmp1, 0 );
    exit( 1 );
  }
  ...
  case OR:
    for ( w = tmp1->sons; w; w = w->next ) {
      ...
      make_instantiate_literals( tmp2, effect->effects );
      make_instantiate_numeric_effects( tmp2, effect->numeric_effects );
      tmp2->next = pseudoAction->effects;
      pseudoAction->effects = tmp2;
      pseudoAction->num_effects++;
    }
    break;
  case AND:
    tmp2 = new_PseudoActionEffect();
