...

{
  ...
  for ( i = 0; i < gnum_predicates; i++ ) {
    ...
    /*  TODO (DRJ)  Release previous memory from SAFE_CALLOC
        Check if we have already allocated on a previous run
        and free/reset if so.
        Do this for the next four SAFE_CALLOCs.
    */
/*
    if (lpos[i]) {
      free(lpos[i]);
 ...
 */   lindex[i] = SAFE_CALLOC( int_pointer, int, size );
  ...
  /*  TODO (DRJ) Release previous memory from SAFE_CALLOC
      ...
      and free/reset if so.
  */
  if (had_hard_template) {
    printf("This should be NULL first time\n");
    free(had_hard_template);
    had_hard_template = NULL;
  }
  ...
  if ( gcmd_line.display_info == 118) {
    ...
    for ( i = 0; i < gnum_operators; i++ ) {
      ...
      for ( a = gactions; a; a = a->next ) {
        if ( ( a->norm_operator && a->norm_operator->pddloperator !=  goperators[i] ) ||
...

{
  ...
  for ( i = 0; i < o->pddloperator->number_of_real_params; i++ ) {
    if ( o->num_removed_vars > r && o->removed_vars[r] == i ) {
...

{
  ...
  for ( i = 0; i < gnum_functions; i++ ) {
    ...
    /*  TODO (DRJ)  Release previous memory from SAFE_CALLOC
        Check if we have already allocated on a previous run
        and free/reset if so.
        Do this for the next two SAFE_CALLOCs.
    */
    if (lf_def[i]) {
      free(lf_def[i]);
      lf_def[i] = NULL;
  ...
  for ( fvs = gf_initial; fvs; fvs = fvs->next ) {
    ...
    if ( !lf_def[lf][adr] ) {
      ...
      grelevant_fluents_name[gnum_relevant_fluents] = SAFE_CALLOC( char*, char, MAX_LENGTH );
    ...
    for ( i = 0; i < gnum_operators; i++ ) {
      ...
      for ( a = gactions; a; a = a->next ) {
        if ( ( !a->norm_operator && !a->pseudo_action ) ||
             ( a->norm_operator && a->norm_operator->pddloperator != goperators[i] ) ||
             ( a->pseudo_action && a->pseudo_action->pddloperator != goperators[i] ) ) {
          continue;
        }
        print_Action( a );
  ...
  while ( a ) {
    if ( a->norm_operator ) {
      ...
      if ( no->num_numeric_preconds > 0 ) {
        a->numeric_preconds_comp = ( no->num_numeric_preconds == 0 ? NULL : ( Comparator * )  calloc( no->num_numeric_preconds, sizeof( Comparator ) ) );
        a->numeric_preconds_lh = ( no->num_numeric_preconds == 0 ? NULL : ( ExpNode_pointer * )  calloc( no->num_numeric_preconds, sizeof( ExpNode_pointer ) ) );
        a->numeric_preconds_rh = ( no->num_numeric_preconds == 0 ? NULL : ( ExpNode_pointer * ) calloc( no->num_numeric_preconds, sizeof( ExpNode_pointer ) ) );
	...
	continue;
      ...
      for ( ne = no->effects; ne; ne = ne->next ) {
        ...
        if ( i < ne->num_conditions ) {/* found unreachable condition: free condition space */
          free( aa->conditions );
          /* TODO (DRJ) added reset to NULL after free */
          aa->conditions = NULL;
          continue;
        }
        ...
        if ( ne->num_numeric_conditions > 0 ) {
          aa->numeric_conditions_comp = ( Comparator * ) calloc( ne->num_numeric_conditions, sizeof( Comparator ) );
          aa->numeric_conditions_lh = ( ExpNode_pointer * ) calloc( ne->num_numeric_conditions, sizeof( ExpNode_pointer ) );
          aa->numeric_conditions_rh = ( ExpNode_pointer * ) calloc( ne->num_numeric_conditions, sizeof( ExpNode_pointer ) );
        ...
        if ( ne->num_numeric_effects > 0 ) {
          aa->numeric_effects_neft = ( NumericEffectType * ) calloc( ne->num_numeric_effects, sizeof( NumericEffectType ) );
          aa->numeric_effects_fl = ( int * ) calloc( ne->num_numeric_effects, sizeof( int ) );
          aa->numeric_effects_rh = ( ExpNode_pointer * ) calloc( ne->num_numeric_effects, sizeof( ExpNode_pointer ) );
    ...
    if ( a->pseudo_action ) {
      ...
      if ( pa->num_numeric_preconds > 0 ) {
        a->numeric_preconds_comp = ( Comparator * ) calloc( pa->num_numeric_preconds, sizeof( Comparator ) );
        a->numeric_preconds_lh = ( ExpNode_pointer * ) calloc( pa->num_numeric_preconds, sizeof( ExpNode_pointer ) );
        a->numeric_preconds_rh = ( ExpNode_pointer * ) calloc( pa->num_numeric_preconds, sizeof( ExpNode_pointer ) );
        a->num_numeric_preconds = 0;
      }
      ...
      for ( pae = pa->effects; pae; pae = pae->next ) {
        ...
        if ( pae->num_numeric_conditions > 0 ) {
          aa->numeric_conditions_comp = ( Comparator * ) calloc( pae->num_numeric_conditions, sizeof( Comparator ) );
          aa->numeric_conditions_lh = ( ExpNode_pointer * ) calloc( pae->num_numeric_conditions, sizeof( ExpNode_pointer ) );
          aa->numeric_conditions_rh = ( ExpNode_pointer * ) calloc( pae->num_numeric_conditions, sizeof( ExpNode_pointer ) );
        ...
        if ( pae->num_numeric_effects > 0 ) {
          aa->numeric_effects_neft = ( NumericEffectType * ) calloc( pae->num_numeric_effects, sizeof( NumericEffectType ) );
          aa->numeric_effects_fl = ( int * ) calloc( pae->num_numeric_effects, sizeof( int ) );
          aa->numeric_effects_rh = ( ExpNode_pointer * ) calloc( pae->num_numeric_effects, sizeof( ExpNode_pointer ) );
          aa->num_numeric_effects = 0;
        }
        for ( i = 0; i < pae->num_numeric_effects; i++ ) {
          ...
          if ( aa->illegal && aa->num_conditions == 0 && aa->num_numeric_conditions == 0 ) {
            break;
          }
          /* that's it ???????????????? - !!
           */
...

{
  ...
  for ( i = 0; i < gnum_fl_conn; i++ ) {
    ...
    if ( grelevant_fluents_lnf[i] == NULL ) {
      ...
      gfl_conn[i].lnf_F = ( int * ) calloc( grelevant_fluents_lnf[i]->num_pF, sizeof( int ) );
      gfl_conn[i].lnf_C = ( float * ) calloc( grelevant_fluents_lnf[i]->num_pF, sizeof( float ) );
  ...
  for ( a = gactions; a; a = a->next ) {
    ...
    for ( i = 0; i < a->num_effects; i++ ) {
      ...
      gef_conn[n_ef].f_PC_comp = ( n_lcond_lpreconds ? ( Comparator * ) calloc( e->num_lnf_conditions + a->num_lnf_preconds, sizeof( Comparator ) ) : NULL );
      gef_conn[n_ef].f_PC_fl = ( n_lcond_lpreconds ? ( int * )  calloc( e->num_lnf_conditions + a->num_lnf_preconds, sizeof( int ) ) : NULL );
      gef_conn[n_ef].f_PC_c = ( n_lcond_lpreconds ? ( float * ) calloc( e->num_lnf_conditions + a->num_lnf_preconds, sizeof( float ) ) : NULL );
