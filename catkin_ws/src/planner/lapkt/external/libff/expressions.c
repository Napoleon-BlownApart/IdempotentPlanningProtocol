  ...
  for ( i = 0; i < gnum_operators; i++ ) {
    ...
    for ( a = gactions; a; a = a->next ) {
      if ( ( !a->norm_operator &&
           ...
           ( a->pseudo_action && a->pseudo_action->pddloperator != goperators[i] ) ) {
        continue;
      }
      print_lnf_Action( a );
    }
...

{
  ...
  action = gactions; p = NULL;
  while ( action ) {
    for ( i = 0; i < action->num_numeric_preconds; i++ ) {
      if ( !translate_divisions( &(action->numeric_preconds_lh[i]) ) ) break;
      push_multiplications_down( &(action->numeric_preconds_lh[i]) );
      if ( !translate_divisions( &(action->numeric_preconds_rh[i]) ) ) break;
      push_multiplications_down( &(action->numeric_preconds_rh[i]) );
    }
    if ( i < action->num_numeric_preconds ) {
      if ( gcmd_line.display_info ) {
    ...
    for ( i = 0; i < action->num_effects; i++ ) {
      actionEffect = &(action->effects[i]);

      for ( j = 0; j < actionEffect->num_numeric_conditions; j++ ) {
        if ( !translate_divisions( &(actionEffect->numeric_conditions_lh[j]) ) ) break;
        push_multiplications_down( &(actionEffect->numeric_conditions_lh[j]) );
        if ( !translate_divisions( &(actionEffect->numeric_conditions_rh[j]) ) ) break;
        push_multiplications_down( &(actionEffect->numeric_conditions_rh[j]) );
      }
      if ( j < actionEffect->num_numeric_conditions ) break;

      if ( actionEffect->illegal ) {
        continue;
      }

      for ( j = 0; j < actionEffect->num_numeric_effects; j++ ) {
        if ( !translate_divisions( &(actionEffect->numeric_effects_rh[j]) ) ) break;
        push_multiplications_down( &(actionEffect->numeric_effects_rh[j]) );
      }
      if ( j < actionEffect->num_numeric_effects ) {
        if ( gcmd_line.display_info ) {
          printf("\nwarning: division by zero in effect rh of ");
          print_Action_name( action );
          printf(". marking effect as illegal.");
        }
        actionEffect->illegal = TRUE;
      }
    }
    if ( i < action->num_effects ) {
      if ( gcmd_line.display_info ) {
   ...
   */
  for ( action = gactions; action; action = action->next ) {
    /* preconds
     */
    action->lnf_preconds_comp = SAFE_CALLOC(Comparator*, Comparator, MAX_LNF_COMPS);
    action->lnf_preconds_lh = SAFE_CALLOC(LnfExpNode_pointer*, LnfExpNode_pointer, MAX_LNF_COMPS);
    action->lnf_preconds_rh = SAFE_CALLOC(float*, float, MAX_LNF_COMPS );
    action->num_lnf_preconds = 0;
    for ( i = 0; i < action->num_numeric_preconds; i++ ) {
      if ( action->num_lnf_preconds == MAX_LNF_COMPS ) {
        printf("\n\nincrease MAX_LNF_COMPS! currently %d\n\n", MAX_LNF_COMPS);
        exit( 1 );
      }
      eq = FALSE;
      if ( action->numeric_preconds_comp[i] == EQ ) {
        eq = TRUE;
        action->numeric_preconds_comp[i] = LEQ;
      }
      put_comp_into_normalized_locals( action->numeric_preconds_comp[i], action->numeric_preconds_lh[i], action->numeric_preconds_rh[i] );
      action->lnf_preconds_comp[action->num_lnf_preconds] = lcomp;
      action->lnf_preconds_lh[action->num_lnf_preconds] = new_LnfExpNode();
      lnf = action->lnf_preconds_lh[action->num_lnf_preconds];
      for ( j = 0; j < lnum_F; j++ ) {
        if ( lC[j] == 0 ) continue;
        if ( lC[j] > 0 ) {
      ...
      action->lnf_preconds_rh[action->num_lnf_preconds] = lc;
      action->num_lnf_preconds++;
      if ( eq ) {
        if ( action->num_lnf_preconds == MAX_LNF_COMPS ) {
          printf("\n\nincrease MAX_LNF_COMPS! currently %d\n\n", MAX_LNF_COMPS);
          exit( 1 );
        }
        action->numeric_preconds_comp[i] = EQ;
        put_comp_into_normalized_locals( GEQ, action->numeric_preconds_lh[i], action->numeric_preconds_rh[i] );
        action->lnf_preconds_comp[action->num_lnf_preconds] = lcomp;
        action->lnf_preconds_lh[action->num_lnf_preconds] = new_LnfExpNode();
        lnf = action->lnf_preconds_lh[action->num_lnf_preconds];
        for ( j = 0; j < lnum_F; j++ ) {
    ...
    for ( i = 0; i < action->num_effects; i++ ) {
      ...
      actionEffect->lnf_conditions_comp = ( Comparator * ) calloc( MAX_LNF_COMPS, sizeof( Comparator ) );
      actionEffect->lnf_conditions_lh = ( LnfExpNode_pointer * ) calloc( MAX_LNF_COMPS, sizeof( LnfExpNode_pointer ) );
      actionEffect->lnf_conditions_rh = ( float * ) calloc( MAX_LNF_COMPS, sizeof( float ) );
      actionEffect->num_lnf_conditions = 0;
      for ( j = 0; j < actionEffect->num_numeric_conditions; j++ ) {
        if ( actionEffect->num_lnf_conditions == MAX_LNF_COMPS ) {
          printf("\n\nincrease MAX_LNF_COMPS! currently %d\n\n", MAX_LNF_COMPS);
          exit( 1 );
        }
        eq = FALSE;
        if ( actionEffect->numeric_conditions_comp[j] == EQ ) {
          eq = TRUE;
          actionEffect->numeric_conditions_comp[j] = LEQ;
        }
        put_comp_into_normalized_locals( actionEffect->numeric_conditions_comp[j], actionEffect->numeric_conditions_lh[j], actionEffect->numeric_conditions_rh[j] );
        actionEffect->lnf_conditions_comp[actionEffect->num_lnf_conditions] = lcomp;
        actionEffect->lnf_conditions_lh[actionEffect->num_lnf_conditions] = new_LnfExpNode();
        lnf = actionEffect->lnf_conditions_lh[actionEffect->num_lnf_conditions];
        for ( k = 0; k < lnum_F; k++ ) {
          if ( lC[k] == 0 ) continue;
          if ( lC[k] > 0 ) {
        ...
        actionEffect->lnf_conditions_rh[actionEffect->num_lnf_conditions] = lc;
        actionEffect->num_lnf_conditions++;
        if ( eq ) {
          if ( actionEffect->num_lnf_conditions == MAX_LNF_COMPS ) {
            printf("\n\nincrease MAX_LNF_COMPS! currently %d\n\n", MAX_LNF_COMPS);
            exit( 1 );
          }
          actionEffect->numeric_conditions_comp[j] = EQ;
          put_comp_into_normalized_locals( GEQ, actionEffect->numeric_conditions_lh[j], actionEffect->numeric_conditions_rh[j] );
          actionEffect->lnf_conditions_comp[actionEffect->num_lnf_conditions] = lcomp;
          actionEffect->lnf_conditions_lh[actionEffect->num_lnf_conditions] = new_LnfExpNode();
          lnf = actionEffect->lnf_conditions_lh[actionEffect->num_lnf_conditions];
          for ( k = 0; k < lnum_F; k++ ) {
      ...
      actionEffect->lnf_effects_neft = ( NumericEffectType * ) calloc( MAX_LNF_EFFS, sizeof( NumericEffectType ) );
      actionEffect->lnf_effects_fl = ( int * ) calloc( MAX_LNF_EFFS, sizeof( int ) );
      actionEffect->lnf_effects_rh = ( LnfExpNode_pointer * ) calloc( MAX_LNF_EFFS, sizeof( LnfExpNode_pointer ) );
      actionEffect->num_lnf_effects = 0;
      for ( j = 0; j < actionEffect->num_numeric_effects; j++ ) {
        if ( actionEffect->num_lnf_effects == MAX_LNF_EFFS ) {
          printf("\n\nincrease MAX_LNF_EFFS! currently %d\n\n", MAX_LNF_EFFS);
          exit( 1 );
        }
        actionEffect->lnf_effects_neft[actionEffect->num_lnf_effects] = actionEffect->numeric_effects_neft[j];
        actionEffect->lnf_effects_fl[actionEffect->num_lnf_effects] = actionEffect->numeric_effects_fl[j];
        lnum_F = 0;
        lc = 0;
        if ( actionEffect->lnf_effects_neft[actionEffect->num_lnf_effects] == DECREASE ) {
          collect_normalized_locals( actionEffect->numeric_effects_rh[j], FALSE );
          actionEffect->lnf_effects_neft[actionEffect->num_lnf_effects] = INCREASE;
        } else {
          collect_normalized_locals( actionEffect->numeric_effects_rh[j], TRUE );
        }
        actionEffect->lnf_effects_rh[actionEffect->num_lnf_effects] = new_LnfExpNode();
        lnf = actionEffect->lnf_effects_rh[actionEffect->num_lnf_effects];
        for ( k = 0; k < lnum_F; k++ ) {
          if ( lC[k] == 0 ) continue;
          if ( lC[k] > 0 ) {
            lnf->pF[lnf->num_pF] = lF[k];
        ...
        actionEffect->lnf_effects_rh[actionEffect->num_lnf_effects]->c = lc;
        actionEffect->num_lnf_effects++;
      }
    }
  }
  ...
  for ( i = 0; i < gnum_numeric_goal; i++ ) {
    ...
    put_comp_into_normalized_locals( gnumeric_goal_comp[i], gnumeric_goal_lh[i], gnumeric_goal_rh[i] );
    ...
    if ( eq ) {
      ...
      put_comp_into_normalized_locals( GEQ, gnumeric_goal_lh[i], gnumeric_goal_rh[i] );
      glnf_goal_comp[gnum_lnf_goal] = lcomp;
      glnf_goal_lh[gnum_lnf_goal] = new_LnfExpNode();
      lnf = glnf_goal_lh[gnum_lnf_goal];
      for ( j = 0; j < lnum_F; j++ ) {
...

{
  ...
  grelevant_fluents_name[gnum_relevant_fluents] = ( char * ) calloc( MAX_LENGTH, sizeof( char ) );
...

{
  ...
  /*  TODO (DRJ) added free and reset to NULL.
      They are not protected by a STATIC and aren't used outside of
        summarize_effects() and
        merge_effects( ActionEffect *e, ActionEffect *e_ )
  */
  free(lA);
  lA = NULL;
  free(lD);
...

{
  ...
  /*  TODO (DRJ) added free and reset to NULL.
      They are not protected by a STATIC and aren't used outside of
        determine_fl_relevance() and
  */

  if(influenced_by) {
    for ( i = 0; i < gnum_real_fl_conn ; i++ ) {
      if(influenced_by[i]) {
        free(influenced_by[i]);
        influenced_by[i] = NULL;
      }
    }
    free(influenced_by);
    influenced_by= NULL;
