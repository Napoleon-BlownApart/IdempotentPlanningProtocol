...
namespace aptk
{
  ...
  void     STRIPS_Problem::clear_if_needed_part_1(STRIPS_Problem& p) {
    std::cout << " Clearing STRIPS_Problem " << '\n';
    std::cout << " Name: " << p.problem_name() << '\n';
    std::cout << " Domain: " << p.domain_name() << '\n';
    std::cout << " Map Size =  " << p.m_fluents_map.size() << '\n';
    std::cout << " Number fluents =  " << p.fluents().size() << '\n';
    std::cout << " Number m_const_fluents =  " << p.m_const_fluents.size() << '\n';

    while (!p.fluents().empty()) {
      Fluent* old_fluent = p.fluents().back();
      p.m_fluents_map.erase(old_fluent->signature());
      p.fluents().pop_back();
      p.m_const_fluents.pop_back();
      delete old_fluent;
    }

    if (!p.m_fluents_map.empty()) {
          std::cout << " Clearing Map WENT WRONG " << '\n';
    } else {
          std::cout << " Clearing Map SUCCESS " << '\n';
    }
    if (!p.fluents().empty()) {
          std::cout << " Clearing fluents WENT WRONG " << '\n';
    } else {
          std::cout << " Clearing fluents SUCCESS " << '\n';
    }
    if (!p.m_const_fluents.empty()) {
          std::cout << " Clearing m_const_fluents WENT WRONG " << '\n';
    } else {
          std::cout << " Clearing m_const_fluents SUCCESS " << '\n';
    }
  }

  void     STRIPS_Problem::clear_if_needed_part_2(STRIPS_Problem& p) {
    std::cout << " Number m_actions =  " << p.m_actions.size() << '\n';
    std::cout << " Number p.m_const_actions =  " << p.m_const_actions.size() << '\n';
    std::cout << " Number p.actions =  " << p.actions().size() << '\n';


    while (!p.actions().empty()) {
//      std::cout << " INACTION  " << '\n';
      Action* new_act = p.actions().back();
      VarVal_Vec m_prec_varval = new_act->prec_varval();
      while (!m_prec_varval.empty()) {
        m_prec_varval.pop_back();
      }
      p.set_num_actions( p.num_actions()-1);
      p.actions().pop_back();
      p.m_const_actions.pop_back();
      // std::make_pair(precs[i], 0)
//      delete new_act;
    }

     if (!p.actions().empty()) {
          std::cout << " Clearing actions WENT WRONG " << '\n';
    } else {
          std::cout << " Clearing actions SUCCESS " << '\n';
    }
    if (!p.m_const_actions.empty()) {
          std::cout << " Clearing m_const_actions WENT WRONG " << '\n';
    } else {
          std::cout << " Clearing m_const_actions SUCCESS " << '\n';
    }
  }
...
//    std::cout << " Number fluents =  " << p.fluents().size() << '\n';
//		if(!p.m_fluents_map[signature]) {
      p.m_fluents_map[signature] = new_fluent->index();
//		} else {
//		  std::cout << signature << " already exists " << p.m_fluents_map[signature] << '\n';
//      printf("**************  I WAS NOT NULL ************************\n");
//      exit(1);
//		}
	...
	void	STRIPS_Problem::set_init( STRIPS_Problem& p, const Fluent_Vec& init_vec )
	{
      ...
      if ( p.m_in_init.size() < p.num_fluents()) p.m_in_init.reserve(p.num_fluents());
